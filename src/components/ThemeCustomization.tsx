/* eslint-disable @next/next/no-img-element */
/* Auto-generated from static HTML — preserve class names for theme/CSS */
export default function ThemeCustomization() {
  return (
    <>
      <div className="body-overlay"></div>
      
      <button type="button"
          className="theme-customization__button w-48-px h-48-px bg-primary-600 text-white rounded-circle d-flex justify-content-center align-items-center position-fixed end-0 bottom-0 mb-40 me-40 text-2xxl bg-hover-primary-700" aria-label="Theme Customization Button">
          <i className="ri-settings-3-line animate-spin"></i>
      </button>
      <div className="theme-customization-sidebar w-100 bg-base h-100vh overflow-y-auto position-fixed end-0 top-0">
          <div className="d-flex align-items-center gap-3 py-16 px-24 justify-content-between border-bottom">
              <div>
                  <h6 className="text-sm dark:text-white">Theme Settings</h6>
                  <p className="text-xs mb-0 text-neutral-500 dark:text-neutral-200">Customize and preview instantly</p>
              </div>
              <button data-slot="button"
                  className="theme-customization-sidebar__close text-neutral-900 bg-transparent text-hover-primary-600 d-flex text-xl">
                  <i className="ri-close-fill"></i>
              </button>
          </div>
      
          <div className="d-flex flex-column gap-48 p-24 overflow-y-auto flex-grow-1">
      
              <div className="theme-setting-item">
                  <h6 className="fw-medium text-primary-light text-md mb-3">Theme Mode</h6>
                  <div className="d-grid grid-cols-3 gap-3 dark-light-mode">
                      <button type="button"
                          className="theme-btn theme-setting-item__btn d-flex align-items-center justify-content-center h-64-px rounded-3 text-xl active"
                          data-theme="light" aria-label="light">
                          <i className="ri-sun-line"></i>
                      </button>
                      <button type="button"
                          className="theme-btn theme-setting-item__btn d-flex align-items-center justify-content-center h-64-px rounded-3 text-xl"
                          data-theme="dark" aria-label="dark">
                          <i className="ri-moon-line"></i>
                      </button>
                      <button type="button"
                          className="theme-btn theme-setting-item__btn d-flex align-items-center justify-content-center h-64-px rounded-3 text-xl"
                          data-theme="system" aria-label="system">
                          <i className="ri-computer-line"></i>
                      </button>
                  </div>
              </div>
      
              <div className="theme-setting-item">
                  <h6 className="fw-medium text-primary-light text-md mb-3">Page Direction</h6>
                  <div className="d-grid grid-cols-2 gap-3">
                      <button type="button"
                          className="theme-setting-item__btn ltr-mode-btn d-flex align-items-center justify-content-center gap-2 h-56-px rounded-3 text-xl" aria-label="LTR">
                          <span><i className="ri-align-item-left-line"></i></span>
                          <span className="h6 text-sm font-medium mb-0">LTR</span>
                      </button>
      
                      <button type="button"
                          className="theme-setting-item__btn rtl-mode-btn d-flex align-items-center justify-content-center gap-2 h-56-px rounded-3 text-xl" aria-label="RTL">
                          <span className="h6 text-sm font-medium mb-0">RTL</span>
                          <span><i className="ri-align-item-right-line"></i></span>
                      </button>
                  </div>
              </div>
      
              <div className="theme-setting-item">
                  <h6 className="fw-medium text-primary-light text-md mb-3">Color Schema</h6>
                  <div className="d-grid grid-cols-3 gap-3">
                      <button type="button"
                          className="color-picker-btn d-flex flex-column justify-content-center align-items-center"
                          data-color="base" aria-label="Base">
                          <span className="color-picker-btn__box h-40-px w-100 rounded-3"
                              style={{backgroundColor: "#25A194"}}></span>
                          <span className="fw-medium mt-1" style={{color: "#25A194"}}>Base</span>
                      </button>
                      <button type="button"
                          className="color-picker-btn d-flex flex-column justify-content-center align-items-center"
                          data-color="red" aria-label="Red">
                          <span className="color-picker-btn__box h-40-px w-100 rounded-3"
                              style={{backgroundColor: "#dc2626"}}></span>
                          <span className="fw-medium mt-1" style={{color: "#dc2626"}}>Red</span>
                      </button>
                      <button type="button"
                          className="color-picker-btn d-flex flex-column justify-content-center align-items-center"
                          data-color="blue" aria-label="Blue">
                          <span className="color-picker-btn__box h-40-px w-100 rounded-3"
                              style={{backgroundColor: "#2563eb"}}></span>
                          <span className="fw-medium mt-1" style={{color: "#2563eb"}}>Blue</span>
                      </button>
                      <button type="button"
                          className="color-picker-btn d-flex flex-column justify-content-center align-items-center"
                          data-color="yellow" aria-label="Yellow">
                          <span className="color-picker-btn__box h-40-px w-100 rounded-3"
                              style={{backgroundColor: "#ff9f29"}}></span>
                          <span className="fw-medium mt-1" style={{color: "#ff9f29"}}>Yellow</span>
                      </button>
                      <button type="button"
                          className="color-picker-btn d-flex flex-column justify-content-center align-items-center"
                          data-color="cyan" aria-label="Cyan">
                          <span className="color-picker-btn__box h-40-px w-100 rounded-3"
                              style={{backgroundColor: "#00b8f2"}}></span>
                          <span className="fw-medium mt-1" style={{color: "#00b8f2"}}>Cyan</span>
                      </button>
                      <button type="button"
                          className="color-picker-btn d-flex flex-column justify-content-center align-items-center"
                          data-color="violet" aria-label="Violet">
                          <span className="color-picker-btn__box h-40-px w-100 rounded-3"
                              style={{backgroundColor: "#7c3aed"}}></span>
                          <span className="fw-medium mt-1" style={{color: "#7c3aed"}}>Violet</span>
                      </button>
                  </div>
              </div>
      
          </div>
      </div>
    </>
  );
}
