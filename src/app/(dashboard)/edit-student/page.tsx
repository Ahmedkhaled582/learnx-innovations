...
import { useStudentDetails, useUpdateStudent } from "@/features/student/hooks/useStudent";
        ...
        >
          {previewUrl ? (
            <div className="d-flex align-items-center gap-2 w-100 px-12">
              ...
              />
              <span className="text-truncate text-xs text-secondary-light">
...
function EditStudentForm() {
  ...
  // Pre-populate form when student details are loaded
  useEffect(() => {
    if (student) {
      setFormData({
        Id: studentId,
        ...
        AcademicYear: student.academicYear || "Jun 2025/2026",
        ...
        CategoryId: student.categoryId || 0,
        ...
        Password: "", // Optional during edits
      });

      if (student.guardianRelation === "Father") {
        setGuardianType("father");
      } else if (student.guardianRelation === "Mother") {
        setGuardianType("mother");
      } else {
        setGuardianType("others");
      }
    }
  }, [student, studentId]);
  ...
  }, [guardianType, formData.FatherName, formData.FatherPhone, formData.FatherOccupation, formData.MotherName, formData.MotherPhone, formData.MotherOccupation]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    payload.append("StudentImage", student?.studentImage || "");
    payload.append("ParentId", String(student?.parentId || 0));
    payload.append("UserId", student?.userId || "");
    ...
    updateStudentMutation.mutate(payload, {
      onSuccess: () => {
        showToast("success", "Student Updated", `Successfully updated student details`);
    ...
    return (
      <div className="text-center py-50">
        <div className="spinner-border text-primary" role="status">
          <span className="visually-hidden">Loading Student...</span>
        </div>
      </div>
    );
  }
  ...
  if (studentError) {
    return (
      <div className="text-center py-50 text-danger">
        Failed to fetch student details. Verify the student ID or try again.
      </div>
    );
  }
          ...
          <div className="shadow-1 radius-12 bg-base h-100 overflow-hidden">
            ...
            <div className="card-body p-20">
              <div className="row gy-3">
                ...
                <DropZoneField label="Student Photo" file={studentPhoto} setFile={setStudentPhoto} existingImagePath={student?.studentImage} required={true} />
              </div>
            </div>
          </div>
        </div>
